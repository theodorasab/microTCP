...
server_microtcp (uint16_t listen_port, const char *file)
{
  ...
  socket = microtcp_socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  ...
  client_addr_len = sizeof(struct sockaddr);
  accepted = microtcp_accept (&socket, &client_addr, &client_addr_len);
  if (accepted < 0) {
...
client_microtcp (const char *serverip, uint16_t server_port, const char *file)
{
  ...
  if (microtcp_connect (&socket, (struct sockaddr *) &sin, sizeof(struct sockaddr_in))
      == -1) {
    perror ("TCP connect");
    exit (EXIT_FAILURE);
  }
